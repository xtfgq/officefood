...
public class AttendanceViewMap extends BaseActivity
...
{    @BindView(R.id.toolbar)
    ...
    protected int getLayoutId() {
        return R.layout.activity_attendance_view_map;
    }
                ...
                try {
                    if (null != loc) {
                        ...
                        if (loc.getErrorCode() == 0) {//可在其中解析amapLocation获取相应内容。
                            location = loc;
                            SPUtils.putString(AttendanceViewMap.this, DatasKey.LOCATION_INFO, gson.toJson(location));
...
//        mTransAnimator.setInterpolator(new AccelerateDecelerateInterpolator());
    ...
    public void onViewClicked() {
        if (null != mList && 0 < mList.size() && null != mAddressAdapter) {
            ...
            intent1.putExtra("Location",poiItem.getLatLonPoint());
            intent1.putExtra("Address", poiItem.getTitle());
            setResult(1, intent1);
            finish();
        }
